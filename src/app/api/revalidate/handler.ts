...
export async function handleSanityRevalidation(
  request: Request,
  ...
  try {
    ...
    invalidatePath("/");
    invalidatePath("/reports");
    invalidatePath("/archive");
    invalidatePath("/sitemap.xml");
    invalidatePath("/feed.xml");
    invalidatePath("/rss.xml");

    if (slug && (payload._type === "post" || payload._type === "page")) {
      invalidatePath(`/${slug}`);
    } else {
      invalidatePath("/[slug]", "page");
    }

    invalidatePath("/category/[slug]", "page");
