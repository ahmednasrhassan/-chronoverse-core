...
export default function AuthorCard({ authorName }: { authorName?: string }) {
  return (
    <section className="my-10 p-5 bg-card border border-border/80 rounded-md flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      <div className="flex items-start gap-4">
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            <Link 
              href="/about" 
              className="text-sm font-bold text-primary hover:text-amber-400 transition font-mono"
            >
              Ahmed Nasr Hassan
            </Link>
      ...
      <div className="flex items-center gap-2 self-end sm:self-center shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-border/60 w-full sm:w-auto justify-end">
        <a
          ...
          className="text-muted hover:text-purple-brand text-[11px] transition font-mono flex items-center gap-1 bg-raised hover:bg-raised px-2.5 py-1.5 rounded border border-border"
        >
          LinkedIn ↗
        </a>
        <a
