 ...
 */
export default function InternalLinksBox({ articles, title = "Recommended Reading" }: InternalLinksBoxProps) {
  if (!articles || articles.length === 0) return null;

  // Clamp to 6-8 items as specified.
  const displayArticles = articles.slice(0, 8);

  return (
    <section className="mt-16 pt-10 border-t border-zinc-800 print:hidden" aria-labelledby="internal-links-heading">
      <h2
        id="internal-links-heading"
        className="text-2xl font-bold text-zinc-100 mb-6 flex items-center gap-2 tracking-tight"
      >
        <span className="text-[#c87d55]">📚</span> {title}
      </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {displayArticles.map((article) => (
          ...
          >
            ...
            <p className="text-xs text-zinc-500 line-clamp-2 mt-3 pt-3 border-t border-zinc-800/50">
              {stripHtml(article.legacyBody || article.bodyContent || article.content || "").substring(0, 90)}
              …
            </p>
          </Link>
        ))}
