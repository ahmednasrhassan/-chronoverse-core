  ...
  return (
    <section className="mt-16 border-t border-border/80 pt-10">
      ...
      <div className="space-y-4 mb-10">
        {comments.length === 0 ? (
          <p className="text-xs text-muted font-mono">No institutional remarks recorded yet.</p>
        ) : (
          comments.map((c) => (
            <div key={c._id} className="p-4 bg-card border border-border/80 rounded-md">
              ...
              <p className="text-sm text-secondary leading-relaxed font-sans">{c.comment}</p>
            </div>
          ))
        )}
      </div>

      <form onSubmit={handleSubmit} className="space-y-4 bg-card/60 p-6 border border-border/70 rounded-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input
            ...
            className="w-full bg-raised border border-border px-3 py-2 text-xs text-secondary focus:outline-none focus:border-amber-500/50 rounded"
          />
          <input
            type="email"
            required
        ...
        <textarea
          ...
          className="w-full bg-raised border border-border p-3 text-xs text-secondary focus:outline-none focus:border-amber-500/50 rounded resize-none"
        />
